...
export class FormUseCase {
    ...
    async createForm(): Promise<ApiResponse<FormConfig>> {
        try {
            ...
            if (err instanceof FormError) throw new Error('Form received is badly structured');
            throw new Error('Server did not respond when creating the form');
        }
    }

    async updateForm(form: FormConfig): Promise<ApiResponse<boolean>> {
        try {
            FormDomainService.validate(form);
            return await this.formRepository.update(form);
        } catch (err: unknown) {
            if (err instanceof FormError) throw new Error('Form to update is badly structured');
